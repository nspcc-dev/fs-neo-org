...
export class TokensComponent implements OnInit {
  ...
  async get_gas_req(formData) {
    ...
    let promisefs = this.http.get(`${environment.neofs_api}/send/gas/${formData.u_address}`).toPromise();
    let gas_tx_nep;
    await promisefs.then((data) => {
  ...
  async get_gas3_req(formData) {
    ...
    let promisefs = this.http.get(`${environment.neofs_api}/send/gas3/${formData.u_address}`).toPromise();
    let gas_tx_nep;
  ...
  async onClickSubmit(formData) {
    try {
      ...
      else if (Neon.is.address(formData.u_address) == false) {
        this.alert_type = "danger"
        this.header = "ERROR:"
        this.message = "Incorrect Neo address"
        this.show = true;
  ...
  async onClickSubmitNeoFS(pub_key) {
    try {
      ...
      if (this.await_result_gas == true) {
        ...
        let tx = await this.get_neofs_req(pub_key);

        let type = "Nep-5 GAS token";
        let resp_check = await this.await_tx(tx, type)

        if (resp_check == 1) {
          this.alert_type_p = "success";
          this.header_p = `${type} has been successfully sended to NeoFS account.`;
          this.message_p = "TX is accepted in block.";
          this.show_p = true;
        }
        else {
